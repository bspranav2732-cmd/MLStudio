...
from ui import format_metric_value
...
def show_comparison_tab(active_problem_type: str) -> None:
    ...
    for run in runs:
        ...
        if active_problem_type == "Regression":
            ...
            gap = abs(extract_numeric_value(train_score) - extract_numeric_value(val_score))
            
            gen_data.append({
                "Model": model_name,
                "Training R²": format_metric_value(train_score, "float"),
                "Validation R²": format_metric_value(val_score, "float"),
                "Gap": f"{gap:.4f}"
            })
        else:
            ...
            gen_data.append({
                "Model": model_name,
                "Training Accuracy": format_metric_value(train_score, "percent_scaled"),
                "Validation Accuracy": format_metric_value(val_score, "percent_scaled"),
                "Gap": f"{gap:.2%}"
            })
    ...
    for run in runs:
        ...
        if active_problem_type == "Regression":
            perf_data.append({
                "Model": model_name,
                "R²": format_metric_value(m.get('R2 Score', 0.0), "float"),
                "RMSE": format_metric_value(m.get('RMSE', 0.0), "float"),
                "MAE": format_metric_value(m.get('MAE', 0.0), "float"),
                "MAPE": format_metric_value(m.get('MAPE', 0.0), "percent_raw"),
            ...
            perf_data.append({
                "Model": model_name,
                "Accuracy": format_metric_value(m.get('Accuracy', 0.0), "percent_scaled"),
                "Precision": format_metric_value(m.get('Precision', 0.0), "percent_scaled"),
                "Recall": format_metric_value(m.get('Recall', 0.0), "percent_scaled"),
                "F1 Score": format_metric_value(m.get('F1 Score', 0.0), "percent_scaled"),
                "Hyperparameters": hp_str,
                "Training Time": f"{t_time:.3f}s"
            })
