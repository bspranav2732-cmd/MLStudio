...
def get_imports_code(config: Dict[str, Any]) -> str:
    ...
    if optimization == "Random Search":
        imports.append("from sklearn.model_selection import RandomizedSearchCV")
...
def get_model_instantiation_code(problem_type: str, model_name: str, hp: Dict[str, Any], config: Optional[Dict[str, Any]] = None) -> str:
    """
    ...
    Args:
        ...
        config (dict, optional): Full experiment configuration.
    ...
    if model_name == "Linear Regression":
        ...
        return (
            f"model = DecisionTreeRegressor(\n" if problem_type == "Regression" else f"model = DecisionTreeClassifier(\n"
        ...
        use_oob = bool(config.get("use_oob")) if config else bool(hp.get("oob_score") or hp.get("use_oob"))
        bootstrap = True if use_oob else hp.get("bootstrap", True)
        oob_score = True if use_oob else hp.get("oob_score", False)
        
        mf_str = repr(max_features)
        ms_str = repr(max_samples)
...
def generate_python_script(context: ExportContext) -> str:
    ...
    try:
        if "project" in st.session_state:
            dataset_name = st.session_state.project.get("dataset", dataset_name)
    except Exception:
        pass
...
'''
    ...
    code += get_model_instantiation_code(problem_type, config.get("model_name"), config.get("hyperparameters", {}), config) + "\n\n\n"
    ...
    optimization = config.get("optimization", "None")
    use_oob = bool(config.get("use_oob")) and config.get("model_name") == "Random Forest"
    
    if split_method == "Train-Test Split":
        if optimization == "Random Search":
            from parameter_spaces import get_parameter_space
            ps = get_parameter_space(config.get("model_name"))
            opt_iters = config.get("opt_iters", 10)
            opt_cv = config.get("opt_cv", 3)
            code += "# ------------------------------------------------------------------------------\n"
            code += "# 6a. Hyperparameter Optimization (Random Search)\n"
            code += "# ------------------------------------------------------------------------------\n"
            code += f"param_distributions = {repr(ps)}\n\n"
            code += f"search = RandomizedSearchCV(\n"
            code += f"    estimator=pipeline,\n"
            code += f"    param_distributions=param_distributions,\n"
            code += f"    n_iter={opt_iters},\n"
            code += f"    cv={opt_cv},\n"
            code += f"    scoring='{scoring}',\n"
            code += f"    n_jobs=1,\n"
            code += f"    verbose=1,\n"
            code += f"    random_state=42\n"
            code += f")\n\n"
            code += "print('Running RandomizedSearchCV on training data...')\n"
            code += "search.fit(X_train, y_train)\n"
            code += "print(f'Best CV score: {search.best_score_:.4f}')\n"
            code += "print(f'Best parameters: {search.best_params_}')\n\n"
            code += "pipeline = search.best_estimator_\n"
            if use_oob:
                code += "# Ensure final pipeline estimator preserves bootstrap=True and oob_score=True\n"
                code += "pipeline.named_steps['model'].set_params(bootstrap=True, oob_score=True)\n\n"
        elif optimization == "Grid Search":
            from parameter_spaces import get_parameter_space
            ps = get_parameter_space(config.get("model_name"))
            opt_cv = config.get("opt_cv", 3)
            code += "# ------------------------------------------------------------------------------\n"
            code += "# 6a. Hyperparameter Optimization (Grid Search)\n"
            code += "# ------------------------------------------------------------------------------\n"
            code += f"param_grid = {repr(ps)}\n\n"
            code += f"search = GridSearchCV(\n"
            code += f"    estimator=pipeline,\n"
            code += f"    param_grid=param_grid,\n"
            code += f"    cv={opt_cv},\n"
            code += f"    scoring='{scoring}',\n"
            code += f"    n_jobs=1,\n"
            code += f"    verbose=1\n"
            code += f")\n\n"
            code += "print('Running GridSearchCV on training data...')\n"
            code += "search.fit(X_train, y_train)\n"
            code += "print(f'Best CV score: {search.best_score_:.4f}')\n"
            code += "print(f'Best parameters: {search.best_params_}')\n\n"
            code += "pipeline = search.best_estimator_\n"
    ...
    if problem_type == "Regression":
        ...
        code += "print('=' * 40)\n"
        code += f"print(f'Holdout Test R^2 Score: {{r2:.4f}}')\n" if split_method == 'Train-Test Split' else "print(f'R^2 Score: {r2:.4f}')\n"
        code += "print(f'RMSE:                  {rmse:.4f}')\n"
        code += "print(f'MAE:                   {mae:.4f}')\n"
        code += "print(f'MAPE:                  {mape:.2f}%')\n"
        if use_oob:
            code += "if hasattr(pipeline.named_steps['model'], 'oob_score_'):\n"
            code += "    print(f'Random Forest OOB R^2: {pipeline.named_steps[\"model\"].oob_score_:.4f}')\n"
        code += "print('=' * 40)\n"
    else:
        ...
        code += f"print('EVALUATION METRICS ({split_method})')\n"
        code += "print('=' * 40)\n"
        code += f"print(f'Holdout Test Accuracy:  {{accuracy:.2%}}')\n" if split_method == 'Train-Test Split' else "print(f'Accuracy:  {accuracy:.2%}')\n"
        code += "print(f'Precision:              {precision:.2%}')\n"
        code += "print(f'Recall:                 {recall:.2%}')\n"
        code += "print(f'F1 Score:               {f1:.2%}')\n"
        if use_oob:
            code += "if hasattr(pipeline.named_steps['model'], 'oob_score_'):\n"
            code += "    print(f'Random Forest OOB Accuracy: {pipeline.named_steps[\"model\"].oob_score_:.2%}')\n"
        code += "print('\\nConfusion Matrix:')\n"
        code += "print(cm)\n"
        code += "print('=' * 40)\n"
