...
def generate_pdf_report(context: ExportContext, filepath: str) -> str:
    ...
    if best_params:
        story.append(Paragraph("Optimized Hyperparameters (via Search)", h2_style))
        hp_rows = [[Paragraph("Hyperparameter", table_header_style), Paragraph("Optimized Value", table_header_style)]]
    ...
    def format_metric(val, is_pct=False):
        ...
        if isinstance(val, str):
            return val
        if is_pct:
            return f"{val:.2%}" if val else "0.00%"
        return f"{val:.4f}" if val else "0.0000"

    metrics_rows = [[Paragraph("Performance Metric", table_header_style), Paragraph("Value", table_header_style)]]
    
    if problem_type == "Regression":
        metrics_rows.append([Paragraph("R² Score (Coefficient of Determination)", table_cell_style), Paragraph(format_metric(metrics.get("R2 Score")), table_cell_style)])
        metrics_rows.append([Paragraph("RMSE (Root Mean Squared Error)", table_cell_style), Paragraph(format_metric(metrics.get("RMSE")), table_cell_style)])
        metrics_rows.append([Paragraph("MAE (Mean Absolute Error)", table_cell_style), Paragraph(format_metric(metrics.get("MAE")), table_cell_style)])
        metrics_rows.append([Paragraph("MAPE (Mean Absolute Percentage Error)", table_cell_style), Paragraph(format_metric(metrics.get("MAPE"), is_pct=True), table_cell_style)])
    else:
        metrics_rows.append([Paragraph("Accuracy", table_cell_style), Paragraph(format_metric(metrics.get("Accuracy"), is_pct=True), table_cell_style)])
        metrics_rows.append([Paragraph("Precision (Weighted Average)", table_cell_style), Paragraph(format_metric(metrics.get("Precision"), is_pct=True), table_cell_style)])
        metrics_rows.append([Paragraph("Recall (Weighted Average)", table_cell_style), Paragraph(format_metric(metrics.get("Recall"), is_pct=True), table_cell_style)])
        metrics_rows.append([Paragraph("F1 Score (Weighted Average)", table_cell_style), Paragraph(format_metric(metrics.get("F1 Score"), is_pct=True), table_cell_style)])

    if "OOB Score" in metrics:
        metrics_rows.append([Paragraph("OOB Score (Out-of-Bag)", table_cell_style), Paragraph(format_metric(metrics.get("OOB Score")), table_cell_style)])
